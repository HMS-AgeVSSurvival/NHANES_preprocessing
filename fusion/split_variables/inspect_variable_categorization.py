...
if __name__ == "__main__":
    categorizer = pd.read_csv(
        "fusion/data/VarDescription_Chirag.csv", usecols=COLUMNS_TO_TAKE_CATEGORIZER
    ).rename(columns=COLUMNS_TO_TAKE_CATEGORIZER)
    categorizer.drop(
        index=categorizer.index[categorizer["variable"].duplicated()], inplace=True
    )
    ...
    for main_category in ["examination", "laboratory", "questionnaire"]:
        ...
        variables_main_category["category"] = categorizer.loc[main_category].set_index(
            "variable"
        )["category"]

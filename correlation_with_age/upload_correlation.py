...
def correlation_with_age_cli(argvs=sys.argv[1:]):
    parser = argparse.ArgumentParser(
        "Compute and upload the correlations between the variables and the age from NHANES dataset"
    )
    parser.add_argument(
        "-mc",
        "--main_category",
...
def correlation_with_age(main_category):
    ...
    for category_feather in tqdm(os.listdir(f"fusion/data/{main_category}")):
        ...
        data_category = pd.read_feather(
            f"fusion/data/{main_category}/{category_feather}"
        ).set_index("SEQN")

        for variable in data_category.columns:
            index_notna = data_category.index[
                data_category[variable].notna()
            ].intersection(age_indexes)
            ...
            if data_category.loc[index_notna, variable].dtype == "object":
                dummies = pd.get_dummies(
                    data_category.loc[index_notna, variable],
                    prefix=variable,
                    drop_first=False,
                    dtype=np.float32,
                )
                ...
                for dummy_variable in dummies.columns:
                    correlation_dummy, p_value_dummy = scipy.stats.pearsonr(
                        dummies.loc[index_notna, dummy_variable], age.loc[index_notna]
                    )
                ...
                correlation, p_value = scipy.stats.pearsonr(
                    data_category.loc[index_notna, variable], age.loc[index_notna]
                )

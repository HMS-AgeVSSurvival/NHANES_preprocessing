...
def casting_cli(argvs=sys.argv[1:]):
    parser = argparse.ArgumentParser(
        "Cast the files from the cleaning steps to dummy float vectors"
    )
    parser.add_argument(
        "-mc",
        "--main_category",
...
def casting(main_category, category):
    ...
    if len(columns_to_dummies) > 0:
        if main_category == "mortality":
            dummies = pd.get_dummies(
                data_category[columns_to_dummies],
                prefix=columns_to_dummies,
                drop_first=False,
                dtype=np.float32,
            )
        else:
    ...
    data_category.astype(np.float32, copy=False).reset_index().to_feather(
...
def casting_demographics():
    ...
    dummies = pd.get_dummies(
        data_category["RIDRETH1"], prefix="RIDRETH1", drop_first=True, dtype=np.float32
    )
    data_category.drop(columns="RIDRETH1", inplace=True)
    data_category[dummies.columns] = dummies

    data_category.astype(np.float32, copy=False).reset_index().to_feather(
        f"casting/data/demographics/demographics.feather"
    )
